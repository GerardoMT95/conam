 ...
 ******************************************************************************/
package it.csi.conam.conambl.filter;
...
/**
 * Il filtro agisce su tutte le richieste in input. Serve a prevenire attacchi di tipo XSRF. L'idea e' che il primo accesso avvenga sulla risorsa di accesso della componente SPA (es. index.html). In quell'occasione il filtro non fa verifiche ma si preoccupa di creare una nuova sessione servlet se non presente, creare un XSRF cookie, metterlo in sessione e passarlo al client. Nelle successive
 * richieste, se la rihciesta e' destinata allo strato di servizi rest, viene effettuata la verifica di presenza/corrispondenza di cookie+header XSRF. Questo comportamento, unito alla corretta gestione client side del protocollo, permette di evitare il richiamo dello strato rest al di fuori del contesto corretto.
 */
public class XSRFProtectionFilter implements Filter {
	 ...
	 * <ul>
	 * <li>viene creato un nuovo token che viene inserito in sessione e restituito come cookie</li>
	 * </ul>
	 * </li>
	 * <li>se la richiesta corrente e' destinata alla parte REST:
	 * <ul>
	 * <li>se la request contiene una coppia di cookie e header XSRF validi e coincidenti con quello corrente (contenuto in sessione) => procedo, altrimenti errore.</li>
	 * </ul>
	 * </li>
	 * </ul>
	 */
			...
			if (!isRestRequest(hreq)) {
				...
				if (isXSRFSessionAlive(session)) {
					...
					if (validXSRFCookieAndHeader(hreq, session)) {
						...
						createResponseSessionInvalid(hresp);
						return;
					}
				} else {
	...
	private void createResponseSessionInvalid(HttpServletResponse hresp) throws IOException {
		ExceptionVO ex = new ExceptionVO(ErrorCode.SESSIONE_NON_VALIDA, "La sessione non è più valida si prega di rieffettuare il login", null);
		ObjectMapper objectMapper = new ObjectMapper();
		hresp.setContentType("application/json");
		hresp.setCharacterEncoding("UTF-8");
		hresp.setStatus(HttpStatus.FORBIDDEN.value());
		hresp.getWriter().write(objectMapper.writeValueAsString(ex));
	}

	/**
	 * E' necessario fare in modo che le risorse (pagine, etc) non siano mantenute nella cache del browser per evitare che, ad un accesso in una sessione successiva, la fase di ingresso (e quindi di ricreazione del token) venga bypassata. Se cio' accadesse le richieste REST sarebbero le prime ad arrivare al server e si ricadrebbe nella casistica di token non ancora inizializzato.
	 * 
	 * @param response
	 */
	 ...
	 * 
	 * @param hreq
	 * @return true se il path rappresenta una richiesta alla parte REST dell'applicazione che per default deve contenere la stringa "restfacade"
	 */
	 ...
	 * 
	 * @param hresp
	 *            la response in cui aggiungere il cookie
	 * @param token
	...
	/**
	 * verifica se la richiesta possiede una coppia cookie/header XSRF validi e coincidenti con quanto memorizzato in sessione (se presente)
	 * 
	 * @param hreq
	 *            la request
	 * @param session
	...
	private boolean validXSRFCookieAndHeader(HttpServletRequest hreq, HttpSession session) {
		...
		return actualActiveToken != null && actualActiveToken.equals(actualRequestHeader) && actualActiveToken.equals(actualRequestCookie);
	}
	 ...
	 * @param hreq
	 *            la request
	 * @return il valore del cookie XSRF, se presente nella request; la stringa vuota se invece non presente.
	 */
	...
	/**
	 * nome del parametro di inizializzazione che serve per disabilitare il meccanismo. Serve negli scenari di sviluppo in cui la parte client non e' deployata nell'ear dei servizi
	 */
