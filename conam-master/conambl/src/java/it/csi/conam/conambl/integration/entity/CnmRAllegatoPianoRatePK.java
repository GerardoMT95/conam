/*******************************************************************************
 * SPDX-License-Identifier: EUPL-1.2
 * Copyright Regione Piemonte - 2020
 ******************************************************************************/
package it.csi.conam.conambl.integration.entity;

import javax.persistence.Column;
import javax.persistence.Embeddable;
import java.io.Serializable;

/**
 * The primary key class for the cnm_r_allegato_piano_rate database table.
 * 
 */
@Embeddable
public class CnmRAllegatoPianoRatePK implements Serializable {
	//default serial version id, required for serializable classes.
	private static final long serialVersionUID = 1L;

	@Column(name="id_allegato", insertable=false, updatable=false)
	private Integer idAllegato;

	@Column(name="id_piano_rate", insertable=false, updatable=false)
	private Integer idPianoRate;

	public CnmRAllegatoPianoRatePK() {
	}
	public Integer getIdAllegato() {
		return this.idAllegato;
	}
	public void setIdAllegato(Integer idAllegato) {
		this.idAllegato = idAllegato;
	}
	public Integer getIdPianoRate() {
		return this.idPianoRate;
	}
	public void setIdPianoRate(Integer idPianoRate) {
		this.idPianoRate = idPianoRate;
	}

	public boolean equals(Object other) {
		if (this == other) {
			return true;
		}
		if (!(other instanceof CnmRAllegatoPianoRatePK)) {
			return false;
		}
		CnmRAllegatoPianoRatePK castOther = (CnmRAllegatoPianoRatePK)other;
		return 
			this.idAllegato.equals(castOther.idAllegato)
			&& this.idPianoRate.equals(castOther.idPianoRate);
	}

	public int hashCode() {
		final int prime = 31;
		int hash = 17;
		hash = hash * prime + this.idAllegato.hashCode();
		hash = hash * prime + this.idPianoRate.hashCode();
		
		return hash;
	}
}
